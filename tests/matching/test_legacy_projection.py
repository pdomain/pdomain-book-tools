...
def test_projection_preserves_later_fragment_review_state() -> None:
    ...
    page = _page((("hel", "lo"),))
    page.lines[0].words[0].ground_truth_text = "kept-left"
    protected_word = page.lines[0].words[1]
    protected_word.ground_truth_text = "kept-right"
    ...
    assert [word.text for word in page.lines[0].words] == ["hel", "lo"]
    assert page.lines[0].words[0].ground_truth_text == "kept-left"
    assert protected_word.ground_truth_text == "kept-right"
...
def test_projection_does_not_combine_manual_split_words() -> None:
    ...
    page = _page((("hel", "lo"),))
    first_word, second_word = page.lines[0].words
    first_word.ground_truth_text = "manual-left"
    second_word.ground_truth_text = "manual-right"
    first_word.ground_truth_match_keys = {"split": True, "legacy_note": "keep"}
    target = legacy_page_to_match_document(page, document_id="ocr-page")
    graph = match_documents(_source_document("hello"), target, policy=_policy())
    ...
    assert [word.text for word in page.lines[0].words] == ["hel", "lo"]
    assert first_word.ground_truth_text == "manual-left"
    assert second_word.ground_truth_text == "manual-right"
    assert first_word.ground_truth_match_keys["split"] is True
    assert first_word.ground_truth_match_keys["legacy_note"] == "keep"

    second_result = project_match_graph_onto_page(
        page,
        graph,
        document_id="ocr-page",
        page_side=LegacyDocumentSide.TARGET,
    ...
    assert second_result.skipped_relation_ids == result.skipped_relation_ids
    assert [word.text for word in page.lines[0].words] == ["hel", "lo"]
    assert first_word.ground_truth_match_keys["split"] is True
...
def test_graph_bridge_skips_visible_hyphen_fragments_across_lines() -> None:
    ...
    page = _page((("op-",), ("posed",)))
    page.lines[0].words[0].ground_truth_text = "kept-op-"
    page.lines[1].words[0].ground_truth_text = "kept-posed"
    target = legacy_page_to_match_document(page, document_id="ocr-page")
    ...
    assert [line.words[0].ground_truth_text for line in page.lines] == [
        "kept-op-",
        "kept-posed",
    ]
    assert result.mutated_relation_ids == ()
...
def test_projection_skips_topology_merge_for_page_review() -> None:
    """A reviewed page keeps its physical words and ground-truth text."""
    page = _page((("hel", "lo"),))
    page.review = ReviewMetadata(validated=True)
    page.lines[0].words[0].ground_truth_text = "page-left"
    page.lines[0].words[1].ground_truth_text = "page-right"
    target = legacy_page_to_match_document(page, document_id="ocr-page")
    graph = match_documents(_source_document("hello"), target, policy=_policy())
    ...
    assert page.review == ReviewMetadata(validated=True)
    assert result.skipped_relation_ids == (
        graph.best_alternative.relations[0].relation_id,
    )
    assert (
        page.lines[0].words[0].ground_truth_match_keys["match_projection_mutation"]
        == "container_review_preserved"
    )


def test_projection_skips_topology_merge_for_line_review() -> None:
    """A reviewed line keeps its physical words and ground-truth text."""
    page = _page((("hel", "lo"),))
    line = page.lines[0]
    line.review = ReviewMetadata(validated=True)
    line.words[0].ground_truth_text = "line-left"
    line.words[1].ground_truth_text = "line-right"
    target = legacy_page_to_match_document(page, document_id="ocr-page")
    graph = match_documents(_source_document("hello"), target, policy=_policy())
    ...
    assert [word.text for word in line.words] == ["hel", "lo"]
    assert [word.ground_truth_text for word in line.words] == [
        "line-left",
        "line-right",
    ]
    assert line.review == ReviewMetadata(validated=True)
    assert result.skipped_relation_ids == (
        graph.best_alternative.relations[0].relation_id,
    )
    assert (
        line.words[0].ground_truth_match_keys["match_projection_mutation"]
        == "container_review_preserved"
    )
