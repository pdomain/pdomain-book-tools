...
def test_constructor_signature_pin():
    ...
    # is now generated by the dataclass machinery; ``blocks`` is the
    # sole ``InitVar`` field. All known callers already use kwargs
    # (verified in the rename pass), so this is a benign reordering.
    # The legacy aliases ``items`` and ``cv2_numpy_page_image`` are
    # appended by the deprecation shim wrapper.
    # Task 3 (page-split-plan): ``image_array`` removed as an InitVar;
    # it is now a read-only property backed by ``_image_array``.
    assert params == [
        "self",
        "width",
        "height",
