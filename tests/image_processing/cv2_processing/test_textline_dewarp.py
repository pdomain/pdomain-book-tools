...
def _lined_page(h=900, w=700, n_lines=14, top=90, gap=55, text=255):
    ...
    for i in range(n_lines):
        ...
        for x0 in range(60, w - 60, 70):
            cv2.rectangle(img, (x0, y), (x0 + 50, y + 10), int(text), -1)
    return img


def test_detect_textlines_recovers_known_lines():
    ...
    centers = sorted(float(l.ys.mean()) for l in lines)
    ...
    assert all(l.width > 0.5 * page.shape[1] for l in lines)


def test_fit_baselines_recovers_quadratic():
...
def test_fit_baselines_skips_degenerate_short_line():
...
def _curved_coeffs(n=12, h=900, w=700, amp=0.00010, top=120, gap=60):
    """n quadratic baselines sharing the same downward bow (c2=amp)."""
    coeffs = []
    for i in range(n):
        c0 = top + i * gap
        coeffs.append(td.QuadCoeffs(c2=amp, c1=-amp * (w - 1), c0=c0))  # min at x=w/2
    return coeffs


def test_vertical_disparity_flattens_curved_baselines():
    h, w = 900, 700
...
def test_apply_disparity_identity_maps_is_noop():
...
def _stair_lines(n=10, h=900, w=700, top=120, gap=70, side="left", step=4.0):
...
def test_remove_short_lines_culls_below_fraction():
...
def test_horizontal_disparity_even_page_references_min_left():
    h, w = 900, 700
    ...
    target = min(l.left for l in lines)
    worst = max(lines, key=lambda l: l.left)
...
def test_horizontal_disparity_odd_page_references_max_right():
    h, w = 900, 700
    ...
    target = max(l.right for l in lines)
    worst = min(lines, key=lambda l: l.right)
    row = int(np.clip(worst.ys.mean(), 0, h - 1))
    col = int(worst.right)
...
def _warp_page_vertically(flat, amp_px=22.0):
...
def _mean_abs_curvature(page, w):
...
def test_round_trip_reduces_baseline_curvature():
    h, w = 1000, 760
...
def test_build_disparity_maps_shapes_and_identity_baseline():
    h, w = 400, 300
