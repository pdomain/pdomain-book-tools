...
class TestCreateFileThumbnail:
    ...
    def test_create_jpg_thumbnail(self, tmp_path: pathlib.Path) -> None:
    ...
    def test_png_target_raises_not_implemented(self, tmp_path: pathlib.Path) -> None:
        src = self._make_source(tmp_path)
        target = pathlib.Path(tmp_path / "thumb.png")
        with pytest.raises(NotImplementedError):
            create_file_thumbnail(src, target)

    def test_unknown_suffix_raises_value_error(self, tmp_path: pathlib.Path) -> None:
        src = self._make_source(tmp_path)
        target = pathlib.Path(tmp_path / "thumb.bmp")
        with pytest.raises(ValueError, match="suffix"):
            create_file_thumbnail(src, target)

    def test_missing_source_raises_file_not_found(self, tmp_path: pathlib.Path) -> None:
