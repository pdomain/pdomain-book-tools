...
The fix moves `Alignment` to `pd_book_tools.image_processing.types`. The
...
def test_alignment_re_exported_from_cv2_canvas_for_back_compat():
    ...
    from pd_book_tools.image_processing.types import Alignment as Alignment_neutral
...
def test_cupy_canvas_does_not_import_cv2_backend():
    ...
    try:
        importlib.import_module("pd_book_tools.image_processing.cupy_processing.canvas")
    except ImportError:
        import pytest

        pytest.skip("cupy not available in this environment")

    assert "pd_book_tools.image_processing.cv2_processing.canvas" not in sys.modules, (
...
def test_alignment_same_identity_across_backends():
    ...
    from pd_book_tools.image_processing.cupy_processing import canvas as cupy_canvas
    from pd_book_tools.image_processing.cv2_processing import canvas as cv2_canvas
    from pd_book_tools.image_processing.types import Alignment as Alignment_neutral
