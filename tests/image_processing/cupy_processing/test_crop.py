...
@pytest.fixture
def cupy_crop(cupy_module: ModuleType) -> tuple[ModuleType, ModuleType]:
...
class TestCupyCropToRectangle:
    def test_basic_crop(self, cupy_crop: tuple[ModuleType, ModuleType]) -> None:
    ...
    def test_invalid_returns_original(
        self, cupy_crop: tuple[ModuleType, ModuleType]
    ) -> None:
        crop_mod, cp = cupy_crop
    ...
    def test_clamps_to_bounds(self, cupy_crop: tuple[ModuleType, ModuleType]) -> None:
        crop_mod, cp = cupy_crop
        img = cp.zeros((10, 10), dtype=cp.uint8)
    ...
    def test_entirely_right_of_image_returns_original(
        self, cupy_crop: tuple[ModuleType, ModuleType]
    ) -> None:
    ...
    def test_entirely_below_image_returns_original(
        self, cupy_crop: tuple[ModuleType, ModuleType]
    ) -> None:
...
class TestCupyCropEdges:
    def test_crop_each_edge(self, cupy_crop: tuple[ModuleType, ModuleType]) -> None:
        crop_mod, cp = cupy_crop
        img = cp.arange(100, dtype=cp.uint8).reshape(10, 10)
        out = crop_mod.crop_edges(img, top=1, bottom=2, left=3, right=4)
        assert tuple(out.shape) == (7, 3)

    def test_no_crop_returns_same_shape(
        self, cupy_crop: tuple[ModuleType, ModuleType]
    ) -> None:
        crop_mod, cp = cupy_crop
    ...
    def test_excessive_crop_raises(
        self, cupy_crop: tuple[ModuleType, ModuleType]
    ) -> None:
        crop_mod, cp = cupy_crop
        img = cp.zeros((10, 10), dtype=cp.uint8)
    ...
    def test_negative_edge_raises(
        self, cupy_crop: tuple[ModuleType, ModuleType], edge: str
    ) -> None:
        crop_mod, cp = cupy_crop
        img = cp.zeros((10, 10), dtype=cp.uint8)
