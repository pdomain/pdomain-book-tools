...
class TestBboxVerticalMidpoint:
    def test_with_pixel_bbox(self, pixel_bbox: BoundingBox) -> None:
        assert pixel_bbox.vertical_midpoint == pytest.approx(50.0)

    def test_with_normalized_bbox(self, normalized_bbox: BoundingBox) -> None:
...
class TestHasUsableBbox:
    def test_valid_bbox(self, pixel_bbox: BoundingBox) -> None:
        assert pixel_bbox.has_usable_coordinates is True

    def test_none_has_no_usable_coordinates(self) -> None:
...
class TestValidateLineConsistency:
    def _make_simple_page(self) -> Page:
        w1 = Word(
            text="hello",
