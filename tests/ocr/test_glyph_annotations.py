...
def test_ligature_kind_values_are_uppercase_strings() -> None:
...
def test_ligature_kind_long_s_t_member_renamed_to_long_st() -> None:
...
def test_ligature_kind_new_members_oe_ae() -> None:
    """OE and AE are new members added per SPA spec 20 §3 (plan #163 decision 4)."""
    assert LigatureKind.OE == "OE"
    assert LigatureKind.AE == "AE"


def test_ligature_kind_is_str_subtype() -> None:
    assert isinstance(LigatureKind.FI, str)


def test_ligature_kind_from_uppercase_string() -> None:
...
def test_ligature_kind_unknown_raises() -> None:
    with pytest.raises(ValueError):
        LigatureKind("unknown_value")


def test_ligature_kind_member_count() -> None:
...
def test_ligature_mark_to_dict_without_span_emits_uppercase() -> None:
    mark = LigatureMark(kind=LigatureKind.ST)
    d = mark.to_dict()
    assert d == {"kind": "ST", "char_span": None}


def test_ligature_mark_to_dict_long_st_emits_new_value() -> None:
...
def test_ligature_mark_to_dict_new_members() -> None:
...
def test_ligature_mark_from_dict_uppercase_kind() -> None:
...
def test_ligature_mark_from_dict_without_span() -> None:
    mark = LigatureMark.from_dict({"kind": "CT"})
    assert mark.kind is LigatureKind.CT
    assert mark.char_span is None


def test_ligature_mark_from_dict_unknown_kind_raises() -> None:
    with pytest.raises(ValueError):
        LigatureMark.from_dict({"kind": "bogus"})


def test_ligature_mark_roundtrip() -> None:
    mark = LigatureMark(kind=LigatureKind.LONG_ST, char_span=(0, 2))
    assert LigatureMark.from_dict(mark.to_dict()) == mark


def test_ligature_mark_roundtrip_no_span() -> None:
...
def test_ligature_kind_legacy_migration_roundtrip() -> None:
...
def test_ligature_kind_legacy_long_s_t_name_migrates_to_long_st() -> None:
...
def test_glyph_annotations_defaults() -> None:
    ga = GlyphAnnotations()
    assert ga.ligatures == []
...
def test_glyph_annotations_explicit() -> None:
...
def test_glyph_annotations_equality_empty() -> None:
...
def test_glyph_annotations_to_dict_empty() -> None:
    ga = GlyphAnnotations()
    d = ga.to_dict()
    assert d == {
        "ligatures": [],
...
def test_glyph_annotations_to_dict_full() -> None:
...
def test_glyph_annotations_source_default_is_human() -> None:
    assert GlyphAnnotations().source == "human"


@pytest.mark.parametrize("src", ["human", "predicted", "human_confirmed"])
def test_glyph_annotations_source_accepts_valid_values(src: GlyphSource) -> None:
    ga = GlyphAnnotations(source=src)
    assert ga.source == src
    assert ga.to_dict()["source"] == src


def test_glyph_annotations_source_invalid_value_raises() -> None:
    with pytest.raises(ValueError):
        GlyphAnnotations(
            source="robot"  # pyright: ignore[reportArgumentType]  # intentionally invalid: asserts ValueError at runtime
        )


def test_glyph_annotations_from_dict_missing_source_defaults_to_human() -> None:
...
def test_glyph_annotations_from_dict_reads_source() -> None:
    ga = GlyphAnnotations.from_dict(
        {
            "ligatures": [],
            "long_s_positions": [],
...
def test_glyph_annotations_from_dict_unknown_source_raises() -> None:
    with pytest.raises(ValueError):
...
def test_glyph_annotations_from_dict_missing_keys_use_defaults() -> None:
    ga = GlyphAnnotations.from_dict({})
    assert ga == GlyphAnnotations()


def test_glyph_annotations_from_dict_full() -> None:
...
def _make_word(text: str, gt: str | None = None) -> Word:
...
def test_validate_passes_clean_word() -> None:
    ga = GlyphAnnotations(
...
def test_validate_gt_contains_ligature_codepoint_raises() -> None:
...
def test_validate_gt_contains_long_s_raises() -> None:
...
def test_validate_ligature_span_out_of_bounds_raises() -> None:
    ga = GlyphAnnotations(
...
def test_validate_ligature_span_empty_raises() -> None:
...
def test_validate_ligature_span_start_gt_end_raises() -> None:
    ga = GlyphAnnotations(
...
def test_validate_ligature_span_none_passes() -> None:
...
def test_validate_long_s_position_oob_raises() -> None:
...
def test_validate_long_s_position_wrong_char_raises() -> None:
...
def test_validate_long_s_position_correct_s_passes() -> None:
    ga = GlyphAnnotations(long_s_positions=[0])
    word = _make_word("shall", "shall")
    ga.validate(word)  # index 0 = 's', must not raise


def test_validate_long_s_position_uppercase_s_passes() -> None:
...
def test_word_to_dict_omits_glyph_annotations_when_none() -> None:
    word = _make_word("stand")
    assert word.glyph_annotations is None
    d = word.to_dict()
...
def test_word_to_dict_includes_glyph_annotations_when_set() -> None:
    ...
    glyph_annotations_dict = d["glyph_annotations"]
    assert isinstance(glyph_annotations_dict, dict)
    ligatures = cast("list[object]", glyph_annotations_dict["ligatures"])
    first_ligature = cast("dict[str, object]", ligatures[0])
    assert first_ligature["kind"] == "ST"


def test_word_to_dict_includes_empty_glyph_annotations() -> None:
...
def test_word_from_dict_roundtrip_without_glyph_annotations() -> None:
...
def test_word_from_dict_roundtrip_with_glyph_annotations() -> None:
    from pdomain_book_tools.ocr.word import Word
...
def test_word_from_dict_roundtrip_with_empty_glyph_annotations() -> None:
