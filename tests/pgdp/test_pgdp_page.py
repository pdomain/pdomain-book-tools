...
def test_remove_proofer_notes() -> None:
...
def test_remove_blank_page() -> None:
    src = "First line\n[Blank Page]\nSecond"
...
def test_convert_pgdp_dashes_precedence() -> None:
...
def test_remove_leading_asterisk_no_following_word_split() -> None:
...
def test_remove_trailing_asterisk_after_em_dash() -> None:
...
def test_fix_footnotes() -> None:
    src = "Line[12] more[3]"
    out = PGDPResults.fix_footnotes(src)
    assert out == "Line12 more3"


def test_fix_footnotes_no_space_before_marker() -> None:
...
def test_fix_pgdp_diacritics_subset() -> None:
    src = "[=A][=a][:E][.C][`U]['y][)E][c,]"
...
def test_fix_pgdp_diacritics_dot_above_does_not_swallow_other_brackets() -> None:
...
def test_convert_straight_to_curly_quotes_cases() -> None:
...
def test_pgdp_page_process_integration(tmp_path: pathlib.Path) -> None:
    text = (
        "[*Note*]\n"
        "[Blank Page]\n"
        "*'Tis -- a test----*\n"
...
def test_pgdp_export_from_json(tmp_path: pathlib.Path) -> None:
...
def test_pgdp_export_from_json_missing_file(tmp_path: pathlib.Path) -> None:
    missing = tmp_path / "nope.json"
    with pytest.raises(FileNotFoundError):
        PGDPExport.from_json_file(missing)


def test_pgdp_export_from_json_directory_not_file(tmp_path: pathlib.Path) -> None:
    # Passing a directory path should raise FileNotFoundError hitting the "Not a file" branch
    with pytest.raises(FileNotFoundError):
        PGDPExport.from_json_file(tmp_path)


def test_pgdp_export_from_json_empty_object(tmp_path: pathlib.Path) -> None:
...
def test_pgdp_export_from_json_empty_object_str_path_prefix(
...
def test_pgdp_export_from_json_path_prefix_str_multiple(tmp_path: pathlib.Path) -> None:
...
def test_pgdp_export_from_json_file_success(tmp_path: pathlib.Path) -> None:
...
def test_pgdp_export_from_json_file_success_str_path(tmp_path: pathlib.Path) -> None:
...
def test_convert_quotes_basic_double() -> None:
    src = '"Hello"'
    out = PGDPResults.convert_straight_to_curly_quotes(src)
    assert (
        out == "\u201cHello\u201d"
...
def test_convert_quotes_nested() -> None:
    src = "\"He said, 'Go.'\""
    out = PGDPResults.convert_straight_to_curly_quotes(src)
    assert (
        out == "\u201cHe said, \u2018Go.\u2019\u201d"
...
def test_convert_quotes_possessive() -> None:
    src = "James' hat"
    out = PGDPResults.convert_straight_to_curly_quotes(src)
    assert out == "James\u2019 hat"  # RIGHT SINGLE QUOTATION MARK


def test_convert_quotes_contraction() -> None:
    src = "don't"
    out = PGDPResults.convert_straight_to_curly_quotes(src)
    assert out == "don\u2019t"  # RIGHT SINGLE QUOTATION MARK


def test_convert_quotes_leading_elision_lowercase() -> None:
    src = "'tis the season"
    out = PGDPResults.convert_straight_to_curly_quotes(src)
...
def test_convert_quotes_leading_elision_variants() -> None:
...
def test_convert_quotes_decade_abbreviation() -> None:
...
def test_convert_quotes_year_abbreviation() -> None:
...
def test_convert_quotes_after_em_dash_double() -> None:
    src = '\u2014"Well"'  # EM DASH
    out = PGDPResults.convert_straight_to_curly_quotes(src)
...
def test_convert_quotes_after_em_dash_single() -> None:
    src = "\u2014'Well"  # EM DASH
    out = PGDPResults.convert_straight_to_curly_quotes(src)
    assert out.startswith("\u2014\u2018Well")  # LEFT SINGLE QUOTATION MARK, EM DASH


def test_convert_quotes_word_final_drop() -> None:
    src = "doin' it"
    out = PGDPResults.convert_straight_to_curly_quotes(src)
    assert out == "doin\u2019 it"  # RIGHT SINGLE QUOTATION MARK


def test_convert_quotes_cause_in_quoted_phrase() -> None:
    src = '"\'Cause he left"'
    out = PGDPResults.convert_straight_to_curly_quotes(src)
