    ...
    def from_tesseract(
        ...
        for page_idx, page_row in page_filtered.itertuples():
            ...
            block_filter = (
                ...
                and tesseract_output["page_num"] == page_idx + 1
            )
            ...
            for block_idx, block_row in block_filtered.itertuples():
                ...
                paragraph_filter = (
                    ...
                    and tesseract_output["page_num"] == page_idx + 1
                    and tesseract_output["block_num"] == block_idx + 1
                )
                paragraph_filtered = tesseract_output.where(paragraph_filter).dropna(
                    how="all"
                )
                for paragraph_idx, paragraph_row in paragraph_filtered.itertuples():
                    left, top, width, height = (
                        paragraph_row["left"],
                        paragraph_row["top"],
                        paragraph_row["width"],
                    ...
                    line_filter = (
                        ...
                        and tesseract_output["page_num"] == page_idx + 1
                        and tesseract_output["block_num"] == block_idx + 1
                        and tesseract_output["par_num"] == paragraph_idx + 1
                    )
                    ...
                    for line_idx, line_row in line_filtered.itertuples():
                        ...
                        word_filter = (
                            ...
                            and tesseract_output["block_num"] == block_idx + 1
                            and tesseract_output["par_num"] == paragraph_idx + 1
                            and tesseract_output["line_num"] == line_idx + 1
                        )
                    ...
                    paragraph = Block(
                        ...
                        child_type=BlockChildType.BLOCKS,
                        block_type=BlockCategory.PARAGRAPH,
                    )
                ...
                block = Block(
                    ...
                    bounding_box=block_bounding_box,
                    child_type=BlockChildType.BLOCKS,
                    block_type=BlockCategory.BLOCK,
                )
                blocks.add(block)
