...
Canonical home for the model-resolution code shared by pd-ocr-cli,
pd-ocr-labeler, and pd-prep-for-pgdp. Each downstream app uses
``pd-book-tools`` for inference; this package factors out the
"how do we find / fetch the .pt files" layer so they don't drift.

Subpackage layout:

- :mod:`pd_book_tools.hf.download` — low-level :func:`hf_download` primitive
  and the :func:`suppress_hf_unauth_warning` context manager.
- :mod:`pd_book_tools.hf.models` — registry constants and the
  ``resolve_*`` / ``prefetch_*`` helpers built on top of the primitive.

All public names are re-exported here so callers can write
``from pd_book_tools.hf import hf_download`` and stay decoupled from the
internal split.
"""
