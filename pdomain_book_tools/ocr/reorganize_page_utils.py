...
See ``docs/architecture/reorganize-page-pipeline.md`` for the per-step heuristic
...
from pdomain_book_tools.ocr.block import (
    Block,
    BlockCategory,
    BlockChildType,
...
def _purge_words_from_blocks(blocks: list[Block], targets: set[int]) -> None:
    ...
    purge logic to avoid pulling :mod:`pdomain_book_tools.ocr.layout_aware_reorg`
    (which depends on layout types) into the geometry pipeline. The
    shared implementation now lives on :mod:`pdomain_book_tools.ocr.block`,
...
# in :mod:`pdomain_book_tools.ocr.dropcap` (Iteration B unification -- that module
...
def stitch_drop_caps(blocks: list[Block], metrics: PageMetrics) -> list[Block]:
    ...
    :func:`pdomain_book_tools.ocr.dropcap.stitch_block_drop_caps`. This shim
    is preserved as the historical entry point.
    """
    from pdomain_book_tools.ocr.dropcap import stitch_block_drop_caps
