...
class Page:
    ...
    after the class body) that translates the renamed legacy kwarg
    ``items=`` → ``blocks=`` and accepts the deprecated
    ``cv2_numpy_page_image=`` alias (assigned via the property setter
    after construction). The legacy aliases will be removed in v1.0.
    ...
    # ``docs/architecture/reorganize-page-pipeline.md`` notes.
    ...
    def __eq__(self, other: object) -> bool:  # pyright: ignore[reportImplicitOverride]
        ...
        Skips the ndarray image-cache fields (``_image_array``,
        ``_thumbnail_array``, ``_cv2_numpy_page_image*``)
    ...
    def image_array(self) -> ndarray | None:
        """Return the cached page image array (None if not loaded)."""
    ...
    def get_image(self, blob_store: BlobStoreProtocol) -> ndarray | None:
        ...
        Caches the decoded ndarray; subsequent calls do not hit the blob store.
        """
        if self._image_array is None and self.image_blob_hash is not None:
            data = blob_store.read(self.image_blob_hash)
            decoded = cv2_imdecode(
                np_frombuffer(data, dtype=np_uint8), cv2_IMREAD_COLOR
            )
            if decoded is None:
                raise ValueError(
                    f"failed to decode image blob {self.image_blob_hash!r} as a PNG"
                )
            self._image_array = decoded
        return self._image_array
    ...
    def get_thumbnail(self, blob_store: BlobStoreProtocol) -> ndarray | None:
        ...
        if self._thumbnail_array is None and self.thumbnail_blob_hash is not None:
            ...
            if decoded is None:
                raise ValueError(
                    f"failed to decode thumbnail blob {self.thumbnail_blob_hash!r} as a PNG"
                )
            self._thumbnail_array = decoded
    ...
    def refresh_page_images(self) -> None:
        ...
        self._cv2_numpy_page_image_page_with_bbox = self._image_array.copy()
        self._add_rect(self._cv2_numpy_page_image_page_with_bbox, self)

        self._cv2_numpy_page_image_blocks_with_bboxes = self._image_array.copy()
        ...
        self._cv2_numpy_page_image_paragraph_with_bboxes = self._image_array.copy()
        ...
        self._cv2_numpy_page_image_line_with_bboxes = self._image_array.copy()
        self._add_rect_recurse(
            self.items,
        ...
        self._cv2_numpy_page_image_word_with_bboxes = self._image_array.copy()
        self._add_rect_recurse(
            self.items,
        ...
        self._cv2_numpy_page_image_matched_word_with_colors = self._image_array.copy()
        for w in self.words:
    ...
    def reorganize_page(
        ...
        *,
        ...
        :mod:`pdomain_book_tools.ocr.reorganize_page_utils`. See
        ``docs/architecture/reorganize-page-pipeline.md`` for the per-step
        heuristics, debug-PNG outputs, and the rationale behind each
        threshold.

        When ``layout`` is supplied, layout-derived hints feed two extra
        passes — see :mod:`pdomain_book_tools.ocr.layout_aware_reorg` for the
        ...
        * ``self.diagnostic_noise_dropped_count`` — convenience int
        ...
        from pdomain_book_tools.ocr.dropcap import detect_and_stitch_drop_caps
...
def _page_init_with_deprecation_shim(
    ...
    **kwargs: object,
    ...
    if cv2_numpy_page_image is not None:
        warnings.warn(
            "Page(cv2_numpy_page_image=...) is deprecated; set 'page.cv2_numpy_page_image = ...' after construction, or use 'page.get_image(blob_store)' for blob-backed lazy loading. The 'cv2_numpy_page_image=' constructor kwarg will be removed in v1.0. (The read-side 'page.cv2_numpy_page_image' property is unchanged.)",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    if cv2_numpy_page_image is not None:
        # ``image_array`` is no longer an InitVar — set via the property
        # setter so refresh_page_images() runs.
