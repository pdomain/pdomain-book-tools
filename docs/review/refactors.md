...
## R-01 — Image operations belong in `image_utilities.py`, not on `Word`, `Block`, and `Page`
...
## R-02 — `Page` uses `@dataclass` but also defines a custom `__init__` — pick one
...
## R-03 — `BoundingBox` mixes geometry with image-processing (imports cv2 directly)
