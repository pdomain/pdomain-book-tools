# Archive — Medium-Severity Bugs (closed)

Closed entries originally tracked in `docs/review/bugs-medium.md`. Moved
here on 2026-05-07 once the active doc was cleared. Resolution markers
(`[FIXED in <sha>]`) and the original entry bodies are preserved
verbatim. Order matches the original review numbering (M-01 → M-30).

All 30 medium-severity items have been processed; the active
`bugs-medium.md` is empty.
