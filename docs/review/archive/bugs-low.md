# Archive — Low-Severity Bugs (closed)

Closed entries originally tracked in `docs/review/bugs-low.md`. Moved
here on 2026-05-07 once the active doc was cleared. Resolution markers
(`[FIXED in <sha>]`, `[STALE — regression-locked in <sha>]`,
`[PARTLY STALE + DEFERRED, regression-locked in <sha>]`) and the
original entry bodies are preserved verbatim. Order matches the
original review numbering (L-01 → L-40).

All 40 low-severity items have been processed; the active
`bugs-low.md` is empty.
