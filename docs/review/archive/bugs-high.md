# Archive — High-Severity Bugs (closed)

Closed entries originally tracked in `docs/review/bugs-high.md`. Moved
here on 2026-05-07 once the active doc was cleared. Resolution markers
(`[FIXED in <sha>]`, `[STALE — verified in <sha>]`) and the original
entry bodies are preserved verbatim. Order matches the original review
numbering (H-01 → H-21).

All 21 high-severity items have been processed; the active
`bugs-high.md` is empty.
