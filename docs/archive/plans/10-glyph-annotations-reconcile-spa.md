...
issue: "ConcaveTrillion/pdomain-book-tools#163"
---

# Plan: Reconcile GlyphAnnotations with pdomain-ocr-labeler-spa spec 20

> **Status**: Draft — awaiting CT approval. **DO NOT implement until approved.**
> **Issue**: ConcaveTrillion/pdomain-book-tools#163
> **Spec cross-ref**: `pdomain-book-tools/docs/specs/05-glyph-annotations.md`
> **SPA spec cross-ref**: `pdomain-ocr-labeler-spa/specs/20-glyph-annotations.md`
...
`#41`, `675ad76`) diverges from the pdomain-ocr-labeler-spa spec 20 §3 in
...
### 2.1 Current state (pdomain-book-tools, shipped)
...
| Dimension | pdomain-book-tools (current) | SPA spec 20 (desired) |
|---|---|---|
| Value casing | lowercase (`"fi"`, `"ct"`) | UPPERCASE (`"FI"`, `"CT"`) |
| `LONG_S_T` rename | `LONG_S_T = "long_s_t"` | `LONG_ST = "LONG_ST"` |
| Members present in pdomain-book-tools only | `FF`, `LONG_S_S`, `LONG_S_I`, `SP`, `QU` | absent |
...
- pdomain-ocr-synth synthetic dataset files (if any were produced; synth is
...
other OCR entity in pdomain-book-tools (`Word`, `Page`, `Block`, etc.).
...
| Option | Pros | Cons |
|---|---|---|
| **A. Keep dataclass in pdomain-book-tools; SPA adapts** | Consistent with all other pdomain-book-tools entities; no new heavy dep in foundation lib | SPA must write an adapter/validator layer from dict → Pydantic model |
| **B. Convert pdomain-book-tools to Pydantic** | SPA consumes directly; no adapter needed | Breaks pdomain-book-tools convention; pulls Pydantic into foundation lib; more churn |
| **C. pdomain-book-tools provides both** | Flexible | Maintenance overhead; two sources of truth |

**Proposed decision: Option A.** The SPA already has a pattern of
consuming pdomain-book-tools dicts and rebuilding them into Pydantic models
...
### Phase 1 — LigatureKind rationalization (pdomain-book-tools)
...
3. **Decision point for CT**: which extra members to keep?
   - **Option K** (keep all): keep `FF`, `LONG_S_S`, `LONG_S_I`, `SP`, `QU`
     as valid pdomain-book-tools members even though SPA spec omits them. They
...
7. Change base from `str, Enum` to `StrEnum` (Python 3.11+) — or keep
   `str, Enum` for 3.10 compat; pdomain-book-tools targets 3.10+. **Decision
   point**: SPA spec uses `StrEnum`; pdomain-book-tools can stay on
...
### Phase 3 — dataclass (no change in pdomain-book-tools)

No change. The SPA adds a thin adapter in its persistence layer:

```python
# In pdomain-ocr-labeler-spa: core/persistence/glyph_adapter.py
def glyph_annotations_from_dict(d: dict) -> GlyphAnnotations:
    """Wrap pdomain-book-tools from_dict into a SPA-side Pydantic model."""
    ...
```
...
5. **dataclass stays** in pdomain-book-tools; SPA adapts — proposed YES.
...
- `pdomain-ocr-labeler-spa/core/persistence/` — Pydantic adapter
- `pdomain-ocr-labeler-spa/specs/20-glyph-annotations.md` — no change needed

---

## 7. Risk and timing

**Low risk for pdomain-book-tools itself** — the glyph epic is new and no
downstream release yet persists `LigatureKind` values. Migration window
is still open.

**Timing**: implement after CT approves this plan. Before pdomain-ocr-labeler-spa
