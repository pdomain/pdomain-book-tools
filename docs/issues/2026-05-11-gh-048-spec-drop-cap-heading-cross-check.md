...
# Spec: Drop-cap recognition Iteration C: heading-OCR cross-check disambiguation
...
- **Read when:** planning or implementing spec: drop-cap recognition iteration c: heading-ocr cross-check disambiguation
- **Search terms:** spec, drop-cap, recognition, iteration, heading-ocr, cross-check, disambiguation
- **Relates to:** [Drop-cap recognition Iteration C: heading-OCR cross-check disambiguation](2026-05-10-gh-005-drop-cap-heading-cross-check.md)

## Summary

This open design record tracks specification and decomposition for parent issue
[#5](2026-05-10-gh-005-drop-cap-heading-cross-check.md). The exported body
...
- Parent issue #5 requires design work before implementation.
...
- **Former URL:** <https://github.com/pdomain/pdomain-book-tools/issues/48>
- **GitHub node ID:** `I_kwDOONYVBs8AAAABB3sS2A`
- **Issue number:** 48
- **GitHub state:** `OPEN`
- **State reason:** None
- **Raw export:** `/tmp/github-issues-migration/pdomain-book-tools/raw/issue-48.json`
...
- Tracks parent feature request #5. The duplicate `Tracks: #5` line in the raw
  body adds no separate fact. The body identifies the parent with the historical
  URL `https://github.com/ConcaveTrillion/pd-book-tools/issues/5`.
...
1. Produce the design specification for parent issue #5.
...
1. Draft a governed design specification from parent record
   [#5](2026-05-10-gh-005-drop-cap-heading-cross-check.md) and current
   repository evidence.
...
- Tracks parent issue [#5](2026-05-10-gh-005-drop-cap-heading-cross-check.md).
